...
include 'config.php';
    ...
    if(!empty($_GET['del']) && !empty($_GET['dp'])){
        $del = $_GET['del'];
        $dp = $_GET['dp'];
        if(($dp == 1 || $dp == 2)){
            $update = "UPDATE warranty SET status = 1,departmentID = $dp WHERE warrantyTrackingNumber = $del AND status=0";
...
<h1>Customer Support Manager Dashboard</h1>
  <h2>Filed Warranty Claims</h2><br>
  ...
  <div 
    <thead>
      <tr>
        ...
        <th>Sale Date</th>
        <th>Send to Finished Goods Store?<th>
        <th>Send to Manufacturing Department?<th>
      </tr>
    </thead>
    <tbody>
    ...
    LEFT JOIN customer ON warranty.customerID = customer.customerID LEFT JOIN department ON warranty.departmentID = department.departmentID WHERE warranty.status = 0"; 
    ...
    <tr>
        ...
        <td><?php echo $date;?></td>
        <td align = "center"><a class="btn btn-warning" href="manager-dashboard.php?del=<?php echo $warrantyTrackingNumber;?>&dp=1"><ongoing>
		</a></td>   
        <td></td>
        <td align = "center"><a class="btn btn-primary" href="manager-dashboard.php?del=<?php echo $warrantyTrackingNumber;?>&dp=2"><completed>
		</a></td>
      </tr>
      </tr>
      <?php }?>
